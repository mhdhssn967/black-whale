...
import { Sparkles, Award, DollarSign, X } from 'lucide-react';
...
export default function ProfileSidebar({
  ...
  const renderScorecardGauge = (label, score, colorClass = "from-indigo-500 to-indigo-600", key = "") => {
    return (
      <div className="bg-slate-950/60 border border-slate-900 rounded-xl p-3.5 flex flex-col relative overflow-hidden">
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="glass-panel rounded-2xl p-6 shadow-lg relative overflow-hidden">
        ...
        <div className="mt-5 p-3 rounded-lg bg-slate-950/80 border border-slate-900/60 text-[10px] text-slate-500 text-center leading-normal">
      ...
      <div className="glass-panel rounded-2xl p-6 shadow-md">
        <h3 className="text-base font-bold text-white mb-4 border-b border-slate-850 pb-3 flex items-center space-x-2">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              {editForm.aiGenerated.investmentHighlights.map((hl, idx) => (
                <div key={idx} className="flex items-start justify-between gap-2 bg-slate-950 p-2 border border-slate-900 rounded-lg text-xs">
                  <span className="flex-1 text-slate-300">{hl}</span>
                  <button 
                    type="button" 
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-slate-950 border border-slate-800 rounded px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-indigo-500"
              />
              <button
                type="button"
          ...
          <ul className="space-y-3.5 pl-0 m-0 list-none">
            {startup.aiGenerated.investmentHighlights.map((hl, idx) => (
              <li key={idx} className="flex items-start space-x-2.5 text-xs md:text-sm text-slate-300">
                <span className="mt-1 flex-shrink-0 w-2 h-2 rounded-full bg-cyan-400"></span>
                <span>{hl}</span>
              </li>
            ))}
          </ul>
      ...
      <div className="glass-panel rounded-2xl p-6 shadow-md">
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
              ...
              <span className="font-mono text-base font-bold text-white">
                ${isEditing ? (
                  <input
                    ...
                    value={editForm.fundingRequirement}
                    onChange={handleInputChange}
                    className="bg-transparent border-b border-slate-850 w-full text-white focus:outline-none"
                  />
                ) : (
            ...
            <div className="bg-slate-950/60 p-3 rounded-xl border border-slate-900 col-span-2 grid grid-cols-2 gap-4">
              <div>
                <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block mb-1">
                  CAPITAL RAISED
                </span>
                <span className="font-mono text-sm font-bold text-emerald-400">
                  ${isEditing ? (
                    ...
                    startup.capitalRaised ? parseInt(startup.capitalRaised).toLocaleString() : "0"
                  )}
                </span>
              </div>
              <div>
                ...
                <span className="font-mono text-sm font-bold text-white">
                  {isEditing ? (
                    ...
                    startup.currentRevenue ? `$${startup.currentRevenue}` : "Pre-Revenue"
                  )}
                </span>
              </div>
            </div>
          ...
          <div className="space-y-2">
            ...
            {isEditing ? (
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                {startup.aiGenerated.intendedUseOfFunds}
              </p>
            )}
          </div>
        </div>
